...
## Protocol support
The panel supports VLESS/WS, XHTTP, Trojan/WS and VMess/WS. Trojan and VMess use the bundled Xray-core bridge on localhost; the public FastAPI service terminates HTTPS/WSS and forwards WebSocket traffic to Xray.

Environment variables:
- `PANEL_NAME`: display name of the panel (default `X4G`).
- `XRAY_VERSION`: Xray-core version used for the optional Trojan/VMess bridge (default `26.3.27`).
- `XRAY_DIR`: persistent directory for the Xray binary/config (default `/data/xray`).
